...
interface ParkingDao {
    ...
    // 찜하기
    @Query("UPDATE parking_table SET isBookmarked = 1 WHERE id = :id")
    suspend fun bookmarkParking(id: String)

    // 찜해제 , 삭제(DELETE)가 아니라 상태 변경(UPDATE)이어야 함
    @Query("UPDATE parking_table SET isBookmarked = 0 WHERE id = :id")
    ...
    @Query("SELECT * FROM parking_table WHERE isBookmarked = 1")
    fun getBookmarkedParkingList(): Flow<List<ParkingEntity>>

    // 동기화 용, 현재 즐겨찾기 된 id들만 가져오기 ( repo 합칠 때 )
    @Query("SELECT id FROM parking_table WHERE isBookmarked = 1")
